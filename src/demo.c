    ...
    int benchmark, int benchmark_layers)
{
    ...
    while(1){
        ++count;
        {
            ...
            printf("calling dd\n");
            if (!benchmark && !dontdraw_bbox){cout<<"calling next"; draw_detections_cv_v3(show_img, local_dets, local_nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output);}
            printf("after dd\n");
